...
<xsl:template match="testResults">
	<html>
		...
		<body>
			...
			<xsl:call-template name="detailAll" />
	...
	<xsl:if test="$allFailureCount > 0">
		...
		<xsl:for-each select="/testResults/*[not(@lb = preceding::*/@lb)]">
			...
			<xsl:if test="$failureCount > 0">
				...
				<table style="table-layout: fixed; word-break: break-all; overflow-wrap: break-word;"  align="center" class="details" border="0" cellpadding="5" cellspacing="2" width="95%">
				<tr valign="top">
					<th>Response</th>
...
<xsl:template name="detailAll">

		<h2>DetailAll</h2>
		<h3>这个地方隐藏了一个将所有请求入参结果解析出来的页面</h3>
		<button onclick="document.getElementById('showtime').setAttribute('style','')">ShowMe</button>
		...
		<xsl:for-each select="/testResults/*[not(@lb = preceding::*/@lb)]">
				...
				<xsl:for-each select="/testResults/*[@lb = current()/@lb]">
					<tr>
						...
						<td><xsl:value-of select="./java.net.URL" /></td>
						<td><xsl:value-of select="./queryString" /></td>
						<td style="white-space: nowrap;text-overflow: ellipsis;overflow: hidden; "><xsl:value-of select="./responseData" /></td>
					</tr>
				</xsl:for-each>
